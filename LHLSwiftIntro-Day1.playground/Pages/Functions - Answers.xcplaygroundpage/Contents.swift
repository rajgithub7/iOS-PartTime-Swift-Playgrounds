...
func pluralizedString(count: Int) -> String {
    ...
    if count == 0 {
        return "There are \(count) bicycles"
    }else if count == 1 {
        return "There is \(count) bicycle"
    } else {
        return "There are \(count) bicycles"
    }
}
...
//: So we want a pluralizeWord function that takes in a singular noun string, a plural noun string, and a count.


func pluralizeWord(singularNoun:String, pluralNoun:String, count: Int) -> String {
